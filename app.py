...
currency_pair_data = {
    "BTCUSD": pd.read_csv(
        DATA_PATH.joinpath("BTCUSD.csv"), index_col=1, parse_dates=["Date"]
    )
}
...
def get_top_bar(
    log_classification='Down (0.89)', rnn_pred=1.0898, sentiment_score=0.85, rnn_rmse=2.56, bayes_error=12.4, cumm_pl=32.43
):
    return [
        get_top_bar_cell("Next Bar Direction", log_classification),
        get_top_bar_cell("Next Bar Close", rnn_pred),
        get_top_bar_cell("News Sentiment", f"{sentiment_score}"),
        get_top_bar_cell("RNN RMSE", rnn_rmse),
        get_top_bar_cell("Bayes Error", bayes_error),
        get_top_bar_cell("Model P/L", f"$ {cumm_pl}"),
    ]


####### STUDIES TRACES ######
def bayesian_network_trace(df, fig, window_size=10, num_of_std=5):
    price = df["close"]
    ...
    trace = go.Scatter(
        x=df.index, y=upper_band, mode="lines", showlegend=False, name="Bayes_upper"
    )

    trace2 = go.Scatter(
        x=df.index, y=lower_band, mode="lines", showlegend=False, name="Bayes_lower"
    )
...
# TODO: Refactor the Interval components for GET request from qh_backend server.py (will contain all Model Predictions and price outputs)
# Dash App Layout
app.layout = html.Div(
    className="row",
    children=[
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.P(
                            """
                            Insert day's models performance table below
                            """
                        ),
                    ],
                ),
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[chart_div(pair) for pair in ["BTCUSD"]],
                ),
                # Panel for orders
                html.Div(
                    id="bottom_panel",
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[html.P("TABot Predictions", className="bottom-dropdown")
        ...
        # Hidden div that stores all clicked charts (BTCUSD, etc.)
...
# TODO: Have to "rewire this logic to work with the TABot pattern predictions"
    ...
    if orders is None or orders is "[]":
        return [
            ...
            html.Div(
                ...
                children=[html.P("No predictions from TABot yet")],
            ),
        ]
    ...
    if orders is not None:
        ...
        for order in orders:
            ...
            if order["status"] == "open":
                openOrders += 1
    return [
        {"label": "5 min", "value": "open"},
        {"label": "15 min", "value": "closed"},
        {"label": "30 min", "value": "open"}
    ]
...
# TODO: Have to fix this logic since this is tied to the top-bar of chart (logistic output etc.), therefore formatting of values need to be corrected
